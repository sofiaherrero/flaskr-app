...
def login_required(view):
    """View decorator. The function wrapped_view checks if a user is loaded and redirects to the login page otherwise.
...
def load_logged_in_user():
    ...
    It is done before each request so we store the user in the g object of that request. """
...
def register():
    ...
    if request.method == 'POST':
        ...
        if error is None:
            ...
            db.commit()
            # url_for generates the url for auth.login
            return redirect(url_for('auth.login'))
